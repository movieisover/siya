...
- **문제 1: 타임아웃 (전체 실패 X의 진짜 원인)**
  - `gh run list` 15개 분석: **실패(X)는 전부 3h30m+ / 성공(체크)은 전부 3h28m 이하** — 예외 0. 즉 X는 100% 타임아웃.
  - 원인: Step 1(시세)+4(수급)+6(종목상태)가 KIS를 **종목당 순차 호출**(총 ~8,300콜) → 정상 실행도 3h23m로 타임아웃(210분=3h30m) 코앞. 조금만 느려지면 사망.
  - **조치(A, 즉효)**: `daily-update.yml` `timeout-minutes: 210 → 330`(5h30m). GitHub 잡 최대 6시간 안. 커밋 푸시 완료. **근본 해결은 아니고(여전히 3시간반) 병렬화는 별도 벤치마크 후 신중히**(KIS 초당 제한·앱키 정지 위험) — 미착수.
- **문제 2: ECOS 환율만 실패 (Step 5, 전체는 try/except 격리라 체크로 뜸)**
  - 진단 반전 (틀린 가설 3개 배제): 키 만료→X(마이페이지 유효기간 2028.06까지 정상) / Secret 오염→X(처음 발급해 한 값만 씀, 바뀔 이력 없음) / 간헐적 실패→X(성공한 실행 10/10 전부 INFO-100, 항상 실패).
  - **확정 원인: ECOS가 GitHub Actions의 클라우드(Azure) IP를 '유해 IP'로 간주해 차단.** 로컬(한국 IP)에서는 `collect_fx.py`가 100% 성공(1.2초). ECOS 게시판에 동일 사례 답변 확인("Open API는 대역 차단 안 하나 유해IP는 개별 차단 가능").
  - **교훈**: "갑자기 실패"는 키/값이 아니라 환경(IP) 변수를 먼저 의심. 전체 성공/실패와 Step 개별 성공은 다름(try/except 격리 때문) — 로그 전수 확인으로 분리.
  - **해결(로컬 수집 전환)**: `daily_update.py`에서 `update_fx_step()` 호출 제거(함수 정의는 보존). 환율은 하루 1건·보조지표·UPSERT라 GitHub 자동화 불필요. `run_fx.bat`(신규) + Windows 작업 스케줄러로 로컬(한국 IP) 수집.
    - **배치 함정 2개**: (a) `conda activate`가 cmd/스케줄러에서 `conda init` 없이 안 됨 → **siya python.exe 직접 호출**(`C:\Users\easte\.conda\envs\siya\python.exe`)로 우회. (b) cp949 콘솔이 이모지를 못 찍어 마지막 print에서 크래시 → `PYTHONIOENCODING=utf-8`. (둘 다 과거 Task Scheduler 이슈와 동일 패턴)
    - 검증: 배치 수동 실행 + 스케줄러 즉시 실행 둘 다 성공(`logs/fx_local.log`에 2회 적재, 8건/0.9초). 커밋 `ef39f9d` 푸시 완료.
  - **잔여 설정(동철님 수동)**: Windows 작업 스케줄러에 `run_fx.bat` 등록(매일/주1회, PC 꺼진 날은 --days 10 UPSERT로 메꿈).
- **파일**: `.github/workflows/daily-update.yml`(타임아웃 330), `src/data/collectors/daily_update.py`(Step 5 호출 제거), `run_fx.bat`(신규).

### 2026-08-27: 배제 게이트 B-2 ②단계 ① ✅ 완료 — 종목 상태 스냅샷(관리종목·거래정지)

- **배경**: 1단계 실익 측정 회신 → 시야트레이더 답변(`docs/회신_배제게이트_B2_답변.md`)에서 **①(거래정지+관리종목) 먼저** 확정. 답변서가 게이트를 두 종류로 분리: **체결 게이트**('살 수 있는가' — 거래정지·정리매매·ADV)와 **팩터 배제 게이트**('부실한가' — 자본잠식·관리종목·환기·계속기업). 거래정지는 전자라 유동성 필터와 같은 자리에 놓기로.
- **🚨 1단계 회신의 사실 정정**: 채택 근거로 적었던 **"추가 API 호출 없이 기존 조회 응답에서 뽑을 수 있다"는 틀렸다.** 구현 전 실측 결과 daily_update의 기존 두 호출(**일봉 FHKST03010100 output1 30키 / 수급 FHKST01010900 output 22키**) **둘 다 상태 플래그 미포함**. 상태 플래그는 **주식현재가 `inquire-price`(FHKST01010100) 전용** → 종목당 1콜 별도. **비용 재산정: 2,773종목 × 0.4s ≈ +19분**(런 ~126분 → ~145분). `timeout-minutes: 210` 여유 있고 레포 Public이라 과금 없음 → **채택 판단 자체는 유효**. 회신 문서에 정정 명시.
- **신설 `stock_status`** (`docs/migrate_stock_status.sql`, **Supabase SQL 에디터 수동 적용 필요**): PK `(stock_code, snapshot_date)` — **날짜별 스냅샷 이력**이 쌓이는 구조(덮어쓰기 아님). 컬럼: `iscd_stat_cls_code`(00그외/51관리/52위험/53경고/54주의/55신용가능/57증거금100%/**58거래정지**/59단기과열) · `mang_issu_cls_code`(Y/N) · `mrkt_warn_cls_code`(00/01/02/03) · `sltr_yn`(정리매매) · `short_over_yn`. RLS는 price_daily·fx_daily와 동일(authenticated SELECT). 플래그 선 종목(~2%)용 부분 인덱스 추가.
  - **설계 원칙(답변서 §6) — 시야는 판정하지 않는다**: `is_excluded` 같은 판정 컬럼 **없음**. KIS 원값 그대로 저장(문자열로 안 품), 빈 문자열만 NULL 정규화. 코드의 `is_notable()`/`describe()`는 **로그 출력 전용, DB 미저장**. 판정 규칙이 시야트레이더에서 바뀌어도 시야를 안 건드리게. (B-1 '원자료만 채운다'와 동일 원칙)
  - **③ 투자경고(`mrkt_warn_cls_code`)·정리매매(`sltr_yn`)도 같이 넣음**: 우선순위는 ③이지만 **동일 응답 필드라 한계비용 0**이고, 답변서가 "배제 판정 말고 값만 실어달라"고 한 요구가 이 테이블 원칙과 동일 → 2차 마이그레이션 회피 목적으로 포함. ②(KIND 환기)·④(DART 계속기업)는 별도 소스라 미착수.
- **수집기 `collect_stock_status.py`(신규)**: `update_stock_status(sb, codes/limit/executor/snapshot_date)` + CLI(`--limit`/`--codes`/`--flagged`). collect_fx와 같은 **단일 출처 패턴**(daily_update가 재사용). 1000행마다 200개씩 배치 upsert, `on_conflict='stock_code,snapshot_date'`. 응답 본문 빈 경우(상폐/미상장)는 오류가 아니라 `skipped`.
- **`daily_update.py` Step 6 추가**: `update_stock_status_step()` — `_ensure_kis_token()` 가드 + `execute_with_retry` executor 주입 + **FX Step 5와 동일하게 try/except 격리**(실패해도 Step 1~5 무영향). 워크플로(`daily-update.yml`) **수정 불필요** — KIS 키가 이미 env에 있고 신규 시크릿 없음.
- **드라이런 검증(테이블 생성 전, 업서트 스텁으로 쓰기 없이)**: 9종목 성공/오류 0. 신라섬유·에이비온 `stat=58 mang=Y`(거래정지+관리) / **진흥기업 `stat=58 mang=N`(거래정지인데 관리종목 아님 — 이번 작업의 핵심 케이스 재현)** / 이브이첨단소재·이노인스트루먼트·핀텔 `stat=51` / 혜인 `stat=53 warn=02` / 삼성전자·SK하이닉스 `stat=55` 플래그 없음.
- **⚠️ 이력의 성질**: 과거 상태는 어느 소스로도 소급 복원 불가 → Step 6이 실패한 날은 **영구히 빈 칸**으로 남는다. 소비자는 "연속된 날짜가 아닐 수 있다" 전제로 읽고, 빠진 날은 직전 스냅샷을 끌어 쓰는 게 현실적. (시야트레이더는 `replay.py` 과거 재현에 이 게이트를 아예 미적용하기로 결정)
- **✅ 첫 스냅샷 적재 완료 (2026-08-27, `logs/stock_status_first_20260827.log`)**: 마이그레이션 Supabase 적용 후 전종목 실행 — **2,773/2,773 성공 · 건너뜀 0 · 오류 0 · 2,773행**.
  - **상태 분포**: 57증거금100% 1,655 / 55신용가능 818 / **58거래정지 131** / **51관리종목 86** / 53투자경고 22 / 54투자주의 16 / 59단기과열 10 / 00그외 35. 그 외 **mang=Y 166 · sltr=Y(정리매매) 3**(알에프세미·시스웍·대신밸런스제17호스팩, 전부 관리종목 겸함) · short_over=Y 10 · warn 01주의 21 / 02경고 29.
  - **🔑 `stat=51`(86) ≠ `mang=Y`(166) — 두 필드를 따로 봐야 한다**: 관리종목이면서 거래정지면 `iscd_stat_cls_code`가 **58로 덮인다**. `stat=51`만으로 관리종목을 세면 절반을 놓침. 소비자에게 회신 문서로 명시.
  - **1단계 측정치 완전 재현**: 684(관리 5 / 거래정지 5, 비관리 거래정지 002780·002880·062970) · 313(관리 2 / 거래정지 1). 오전 단건 조회 결과와 DB 적재 후 숫자가 일치.
  - **네이버 교차검증 차이 3건 전부 규명(불일치 아님, 범위 차이)**: 네이버에만 2건(373790·454180 = **KIWOOM ETF** — 시야 `stocks`는 보통주만이라 범위 밖) / DB에만 1건(**269620 시스웍**, `mang=Y stat=51 sltr=Y` 정리매매 — 네이버가 누락. **KIS 커버리지가 더 넓음** 확인).
  - **`00 그외` 35건 = 정상**: `stat='00'`과 `mrkt_warn` NULL이 정확히 같은 집합. 현대홈쇼핑·신세계푸드 등 정상 보통주(우선주 0건). `00`은 KIS 정의 정상값이지 결측 아님, 플래그 미해당이라 배제 로직 무영향.
- **남은 절차**: 매일 16:00 daily_update Step 6이 자동 갱신 → **오늘부터 이력 축적 시작.** 그 다음이 ②(KIND 투자주의환기, 주1회, **조인 실패 로깅 필수**) → ④(DART 계속기업 강조사항, 연1회 3~4월).
- **파일**: `docs/migrate_stock_status.sql`, `src/data/collectors/collect_stock_status.py`(신규), `daily_update.py`(Step 6), `docs/회신_배제게이트_B2_2단계.md`.

### 2026-08-27: 배제 게이트 B-2 — 1단계 실익 측정 완료 (→ 2단계 ①로 확정, 위 기록 참고)

- **배경**: 시야트레이더 핸드오프(`docs/핸드오프_배제게이트_B2.md`). 자동매매 하드 배제 게이트가 반쪽(자본잠식만 적용)이라 감사 비적정·관리종목·상폐사유를 추가하려는데, **유동성 통과 684종목 중 실제로 몇 개나 걸리는지 모른다** → 무거운 수집 전에 실익부터 재라는 요청. **읽기 전용 측정만 수행, DB·수집기 무변경.**
- **결과 (요청 숫자)**: 684 중 **관리종목 5 + 투자주의환기 3 = 8종목(1.2%)**. 313(F게이트) 중 3종목. **최종 선정 20종목 중 0.**
- **🚨 측정하다 발견한 더 급한 것 — 거래정지**: 684 중 **거래정지 5종목**이고 그중 **3종목(진흥기업·대유에이텍·한국첨단소재)은 관리종목이 아니다.** 즉 관리종목 게이트만 넣으면 안 잡힌다. 신라섬유(001000)는 거래정지인데 **313에도 포함** — 실매매면 주문 자체가 안 나가는 종목이 유니버스에 있다. 팩터 품질이 아니라 체결 가능성 문제라 성격이 다름.
- **감사의견 = DART 정형 API로 가능, 단 실익 0**: `accnutAdtorNmNdAdtOpinion`(정기보고서 주요정보 — 회계감사인의 명칭 및 감사의견). 첨부 감사보고서 파싱 불필요. `adt_opinion`/`emphs_matter`/`adtor`/`core_adt_matter` 제공.
  - 684 전수 조회(684콜, 오류 0) → **비적정(한정·부적정·거절) 0종목**. 당연 — 비적정이면 즉시 거래정지·상폐절차라 **60일 ADV 10억을 통과할 수 없다. 유동성 게이트가 이미 감사의견 게이트의 대리 역할.**
  - 건진 것: **계속기업 불확실성 강조사항 3종목**(삼화전자·이노인스트루먼트·로킷헬스케어) — 의견은 적정인데 감사인이 경고 단 케이스.
  - **함정 2개**: ①연결/별도 × 당기/전기 행이 중복인데 `fs_div` 구분 필드가 없음 → `bsns_year`에 '(당기)' 포함 행만 취해 dedupe. ②**의견 필드 빈값 21종목**(현대차·대한항공·한국콜마 등 대형주 포함) = **공시 서식 편차이지 부실 신호 아님** — 빈값을 위험으로 처리하면 대형주가 배제된다.
- **소스 판단**:
  - **관리종목·거래정지 → KIS 권장**. `inquire-price`(FHKST01010100) 응답에 `mang_issu_cls_code`(관리 Y/N) + `iscd_stat_cls_code`(51관리/52위험/53경고/**58거래정지**/55신용가능) + `mrkt_warn_cls_code` + `sltr_yn`이 **이미 들어있다**. 추가 호출 없이 daily_update에 필드만 얹으면 됨. 스크래핑 아님.
    - **교차검증**: 네이버 금융 관리종목 전체(167) ∩ 684 = 5종목, KIS `mang=Y` 5종목과 **완전 일치(불일치 0)**.
  - **투자주의환기 → KIND 스크래핑밖에 없음**(KIS에 필드 없음; `invt_caful_yn`은 다른 개념, 전종목 N). `kind.krx.co.kr/investwarn/hwangiissue.do` POST `method=searchHwangiIssueSub`, **EUC-KR**, 현재 91종목, 지정일·사유 포함. **약점: 종목코드가 없어 종목명 조인** — 91중 6개가 `stocks` 종목명과 불일치(개명 종목, FDR 확인 결과 전부 684 밖이라 이번 숫자엔 무영향). 채택 시 조인 실패 로깅 필수.
  - KIND URL은 추측으로 못 찾음 — `main.do?method=loadInitPage`의 nav 링크에서 발견(`hwangiissue`=환기, `tradinghaltissue`=거래정지, `delcompany`=상폐). `adminissue.do`의 `...Main` URL은 브라우저 제한 에러페이지를 주지만 `...Sub` POST는 동작.
- **⚠️ 지정일/해제일 — 소급 복원 불가**: KIS는 현재 스냅샷만(지정일 없음). KIND는 지정일 제공. 어느 소스든 **과거 임의 시점 상태를 소급 복원할 방법이 없다** → `--as-of` 과거 재현엔 적용 불가, 지금부터 매일 스냅샷을 쌓아야 미래의 재현이 가능. 핸드오프 §2대로 "과거 재현 미적용" 명시 방침 유지.
- **2단계 제안(실익 순 재배열)**: ①거래정지+관리종목(KIS, daily_update) 최우선 → ②투자주의환기(KIND 주1회) → ③**투자경고 판단 요청**(684 중 13, 313 중 6으로 관리종목보다 많으나 부실이 아니라 단기 급등 시장경보 — 배제 취지엔 안 맞지만 증거금100%·매매정지 리스크라 체결 관점에선 걸러야 할 수도) → ④감사의견은 미루거나 계속기업 강조사항만 연1회로 축소.
- **684 목록 확보 방법**: 시야트레이더 `run_selection.py`를 그대로 실행하되 `factors.compute_factors`를 래핑해 유동성+발행주식수 게이트 직후 snap을 덤프(**원본 코드 무수정**). 684행 = `funnel_20260827.csv`의 '발행주식수 확보' 단계와 일치.
- **회신**: `docs/회신_배제게이트_B2_1단계.md` — 1단계 숫자 + 감사의견 API 가부 + 소스 판단 + 2단계 제안. **결정 대기 2건**: 투자경고 배제 여부, 감사의견 포함 여부.
- **상태**: 2단계 미착수. 시야트레이더 회신 받고 저장 스키마 정한 뒤 진행.
