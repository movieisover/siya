...
- [ ] **FX(환율) 분석 — 향후 후보** (2026-06-04 논의, 미착수)
  - 배경: 일부 종목은 원/달러 환율과 상관관계 높음(수출주=양의 상관, 항공·정유 등=음의 상관). retail에 직관적인 보조지표로 가치 있음.
  - 3개 기능 묶음:
    1. **환율 민감도 지표**: 종목 일별수익률 vs 원/달러 일별변동률의 상관계수 + 회귀계수(FX 베타, "환율 +1% → 주가 ±X%"). 둘 다 표시(상관=신뢰도, 베타=크기). 상관 약하면 회색·"참고용" 처리.
    2. **산점도**(펼치기/ⓘ): x=환율변동, y=주가변동 + 회귀선. 베타·상관을 시각적으로 납득시키고 과신 방지.
    3. **캔들차트 "환율" 토글**(이동평균 버튼처럼): 환율을 리베이스(시작점=100) 정규화해 주가 위에 오버레이 → 변동경향 시각 비교. 음의 상관 종목용 "반전(invert)" 옵션 고려.
...
- **재수집 현황(미완)**: **2,447종목 완료**. DART 일 10,000건 한도로 ~170종목(리스트 꼬리 SPAC/소형주) 미완 → **2026-06-20 이후 `python collect_financials.py --remigrate`로 마저 채울 것**. 미완 종목은 그때까지 EPS가 전체순이익으로 폴백(정상 표시, 비지배 큰 종목만 약간 차이).
